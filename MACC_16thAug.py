...
if uploaded_file:
    ...
    projects.columns = [col.strip() for col in projects.columns]  # Strip whitespace
else:
...
type_options = sorted(projects['Project Type'].unique())
...
if len(filtered) > top_n:
    filtered = filtered.sort_values(by='Emissions Saved (tCO₂e)', ascending=False).head(top_n)
...
if not filtered.empty:
    ...
    fig = px.bar(
        ...
        hover_data=['Project Type', 'Timeline (years)', 'Emissions Saved (tCO₂e)'],
        labels={'Cost per tonne (₹)': 'Marginal Cost (₹/tCO₂e)', 'Project Name': 'Project'},
